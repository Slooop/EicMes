...
namespace MES.Server.SQLServerDAL
{
	...
	/// 数据访问类:OQC_Pack3D
	/// </summary>
	public partial class OQC_Pack3D:IOQC_Pack3D
	{
		public OQC_Pack3D()
		{}
		#region  BasicMethod
		...
		public bool Add(MES.Server.Model.OQC_Pack3D model)
		{
			...
			strSql.Append("insert into OQC_Pack3D(");
			...
			parameters[44].Value = model.ID_Key;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update OQC_Pack3D set ");
			strSql.Append("SN=@SN,");
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from OQC_Pack3D ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public MES.Server.Model.OQC_Pack3D DataRowToModel(DataRow row)
		{
			MES.Server.Model.OQC_Pack3D model=new MES.Server.Model.OQC_Pack3D();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM OQC_Pack3D ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from OQC_Pack3D T ");
